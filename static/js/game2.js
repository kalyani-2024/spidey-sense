 ...
 * City reader comment, or a News classified ad); Home hints roughly where. The
 * visible Archive rows are all red-herrings whose numbers do nothing. So you must
 * actually READ to find the number -- spamming the list gets you nowhere.
 *
 * The Green Goblin heckles you; a ticker, ads and comments make the site feel
 * alive; a spider-sense hint nudges anyone who stalls. Asset-free, scoped g2-.
 */
(function () {
  "use strict";

  var HINT_DELAY = 16000;
  ...
  // where the case number hides this play; how Home points; the hint reminder
  var CLUES = {
    corrections: { section: "opinion", ptr: "buried in today&#39;s <b>Corrections</b>, down the <b>Opinion</b> page", target: '#g2-nav [data-goto="opinion"]', msg: "Jonah slipped it into the Corrections &mdash; read the Opinion page&hellip;", where: "in the Opinion Corrections" },
    caption: { section: "home", ptr: "in the small print under the <b>front-page photo</b>", target: "#g2-clue", msg: "It&#39;s in the photo caption, right on the front page&hellip;", where: "under the front-page photo" },
    comment: { section: "city", ptr: "let slip by some reader down in the <b>City</b> comments", target: '#g2-nav [data-goto="city"]', msg: "A reader quoted it &mdash; read the City comments&hellip;", where: "in a City reader comment" },
    ad: { section: "news", ptr: "printed in a fool&#39;s classified <b>ad</b> in the <b>News</b> section", target: '#g2-nav [data-goto="news"]', msg: "Somebody ran an ad about it &mdash; check the News classifieds&hellip;", where: "in a News classified ad" }
  };

  // ---- runtime state ----------------------------------------------------
  var app, screen, urlBar, nav, dateEl;
  var hintTimer = null, urlTyper = null, won = false, goblinShown = false;
  var buriedCase = "", buriedNum = 0, buriedSlug = "", archiveRows = [], clueSpot = "corrections", lookupWrongs = 0, goblinTaunted = false;

  function ri(a, b) { return Math.floor(Math.random() * (b - a + 1)) + a; }
  function pick(arr) { return arr[ri(0, arr.length - 1)]; }
  function caseTag() { return '<span class="g2-case">' + buriedCase + "</span>"; }
  ...
  function buildPuzzle() {
    ...
    // The buried file is UNLISTED. Every visible row is a decoy whose number
    // does nothing -- so scanning/clicking the list can never reach the 404.
    var rows = [];
  ...
  function pageHint(pageId) {
    var c = CLUES[clueSpot];
    if (pageId === "archive") return { sel: "#g2-lookup", msg: "&#128374; Type the case number here &mdash; it&#39;s " + c.where + "&hellip;", delay: 20000 };
    if (pageId === c.section) return { sel: "#g2-clue", msg: "&#128374; There&#39;s the case number &mdash; now punch it into the Archive lookup&hellip;" };
    return { sel: c.target, msg: "&#128374; " + c.msg };
  }
  ...
  function clearHint() {
    ...
    app.classList.remove("is-tingling");
    var lit = app.querySelectorAll(".g2-hint");
    for (var i = 0; i < lit.length; i++) lit[i].classList.remove("g2-hint");
  }
  function armHint(pageId) {
    if (pageId === "notfound") return;
    var h = pageHint(pageId);
